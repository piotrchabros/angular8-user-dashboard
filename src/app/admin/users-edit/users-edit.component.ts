...
export class UsersEditComponent implements OnInit {
  ...
  saveForm() {
    ...
    if (this.user.password.trim().length > 0 && this.user.password.trim() === this.userForm.get('confirmPassword').value) {
      this.usersService.updateUser(this.user, this.id).subscribe(data => {
